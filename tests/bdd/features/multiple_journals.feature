...
Feature: Multiple journals
    ...
    Scenario: Loading a config with two journals
        ...
        Then the output should be
            ...
            """
        When we run "alog work -99 --short"
        Then the output should be empty

    Scenario: Write to default config by default
        Given we use the config "multiple.yaml"
        When we run "alog this goes to default"
        When we run "alog -99 --short"
    ...
    Scenario: Write to specified journal
        Given we use the config "multiple.yaml"
        When we run "alog work a long day in the office"
        When we run "alog -99 --short"
        Then the output should be
            """
            2013-06-09 15:39 My first entry.
    ...
    Scenario: Write to specified journal with a timestamp
        Given we use the config "multiple.yaml"
        When we run "alog work 23 july 2012: a long day in the office"
        When we run "alog -99 --short"
        Then the output should be
            """
            2013-06-09 15:39 My first entry.
   ...
   Scenario: Create new journals as required
        ...
        When we run "alog ideas 23 july 2012: sell my junk on ebay and make lots of money"
        When we run "alog ideas -99 --short"
        Then the output should be
            """
   ...
   Scenario: Don't crash if no default journal is specified
        ...
        When we run "alog a long day in the office"
        Then the output should contain "No 'default' journal configured"

   Scenario: Don't crash if no file exists for a configured encrypted journal
        Given we use the config "multiple.yaml"
        When we run "alog new_encrypted Adding first entry" and enter
            """
            these three eyes
            these three eyes
            n
   ...
   Scenario: Read and write to journal with emoji name
        Given we use the config "multiple.yaml"
        When we run "alog ✨ Adding entry to sparkly journal"
        When we run "alog ✨ -1"
