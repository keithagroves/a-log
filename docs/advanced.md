...
`alog` has a wide variety of options that can be customized through the config file,
...
You can configure `alog`to use with multiple journals (eg.
...
The `default` journal gets created the first time you start `alog`
Now you can access the `work` journal by using `alog work` instead of
`alog`, eg.

``` sh
alog work at 10am: Meeting with @Steve
alog work -n 3
```

will both use `~/work.txt`, while `alog -n 3` will display the last
three entries from `~/journal.txt` (and so does `alog default -n 3`).

You can also override the default options for each individual journal.
If your `alog.yaml` looks like this:
...
the top level of `alog.yaml`, just make sure that at the very least
...
alog --config-override journals.todo "$(git rev-parse --show-toplevel)/todo.txt" todo find my towel 

# Pass multiple overrides 
alog --config-override display_format fancy --config-override linewrap 20 \
--config-override colors.title green
```

### Using an alternate config

You can specify an alternate configuration file for the current instance of `alog` using `--config-file CONFIG_FILE_PATH` where
`CONFIG_FILE_PATH` is a path to an alternate `alog` configuration file. 

#### Examples:

``` sh
# Use personalised configuration file for personal journal entries
alog --config-file ~/foo/alog/personal-config.yaml

# Use alternate configuration file for work-related entries
alog --config-file ~/foo/alog/work-config.yaml
