...
`alog` is a simple journal application for the command line.

You can use it to easily create, search, and view journal entries. Journals are
stored as human-readable plain text, and can also be encrypted using [AES
encryption](http://en.wikipedia.org/wiki/Advanced_Encryption_Standard).

`alog` has most of the features you need, and few of the ones you don't.

## Plain Text

`alog` stores each journal in plain text. You can store `alog` files anywhere,
...
To make it easier to find entries later, `alog` includes support for inline tags
...
`alog` includes support for the creation of multiple journals, each of which
...
entries at a time. `alog` can easily find the entries you want so that you can
read them or edit them.

## Support for External Editors

`alog` plays nicely with your favorite text editor. You may prefer to write
journal entries in an editor. Or you may want to make changes that require a
more comprehensive application. `alog` can filter specific entries and pass them
to the [external editor](./external-editors.md) of your choice.

## Encryption
  
`alog` includes support for [AES
...
`alog` makes it easy to import entries from other sources. Existing entries can
be exported in a variety of [formats](./formats.md).

## Multi-Platform Support

`alog` is compatible with most operating systems. You can [download](./installation.md) it using one
of a variety of package managers, or you can build from source.

## Open-Source

`alog` is written in [Python](https://www.python.org) and maintained by a
