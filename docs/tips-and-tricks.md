...
This page contains tips and tricks for using `alog`, often in conjunction
...
`alog @alberto` will filter the journal to only entries containing the
...
```sh
alog @fixed -starred -n 10 -to "jan 2013" --short
```
...
```sh
alog -from "jan 1 2013" -to "dec 31 2013" | wc -w
```
...
```sh
expr $(alog --export text | wc -w) / $(alog --short | wc -l)
```

This will first get the total number of words in the journal and divide
it by the number of entries (this works because `alog --short` will
print exactly one line per entry).

## Importing older files

If you want to import a file as an entry to `alog`, you can just do `alog < entry.ext`. But what if you want the modification date of the file to
be the date of the entry in `alog`? Try this

```sh
echo `stat -f %Sm -t '%d %b %Y at %H:%M: ' entry.txt` `cat entry.txt` | alog
```

The first part will format the modification date of `entry.txt`, and
then combine it with the contents of the file before piping it to alog.
If you do that often, consider creating a function in your `.bashrc` or
`.bash_profile`

```sh
alogimport () {
  echo `stat -f %Sm -t '%d %b %Y at %H:%M: ' $1` `cat $1` | alog
}
```
...
### 1. Use the `--template` command line argument and the default $XDG_DATA_HOME/alog/templates directory

`$XDG_DATA_HOME/alog/templates` is created by default to store your templates! Create a template (like `default.md`) in this directory and pass `--template FILE_IN_DIR`.

```sh
alog --template default.md
```
...
```sh
alog --template /tmp/template.md
```

### 3. Set a default template file in `alog.yaml`
...
!!! tip
    ...
    command: `alog -n 1` (Check out [Formats](./formats.md) for more options).
...
`alog.yaml` [config file](../advanced/#configuration-file).
...
```sh
alog -on "$(alog --short | shuf -n 1 | cut -d' ' -f1,2)"
```


## Launch a terminal for rapid logging
You can use this to launch a terminal that is the `alog` stdin prompt so you can start typing away immediately.

```bash
alog --config-override editor ""
```

Bind this to a keyboard shortcut.

Map `Super+Alt+J` to launch the terminal with `alog` prompt
...
For more information on how `alog` outputs your entries in Markdown, please visit the [Formats](./formats.md) section.
