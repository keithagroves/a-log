...
from alog.config import DEFAULT_JOURNAL_KEY
from alog.config import get_config_path
from alog.config import get_default_colors
from alog.config import get_default_config
from alog.config import get_default_journal_path
from alog.config import load_config
from alog.config import save_config
from alog.config import verify_config_colors
...
from alog.prompt import yesno
from alog.upgrade import is_old_version
...
def upgrade_config(config_data: dict, alt_config_path: str | None = None) -> None:
    ...
    config file accordingly. This essentially automatically ports alog installations
    ...
    and current alog version, and if so, update the config file accordingly.
...
def find_default_config() -> str:
    config_path = (
        ...
        else os.path.join(home_dir(), ".alog_config")
    )
    return config_path
...
def load_or_install_alog(alt_config_path: str) -> dict:
    """
    If alog is already installed, loads and returns a default config object.
    If alternate config is specified via --config-file flag, it will be used.
    Else, perform various prompts to install alog.
    """
    config_path = (
    ...
    if os.path.exists(config_path):
        ...
        if is_old_version(config_path):
            from alog import upgrade

            upgrade.upgrade_alog(config_path)

        upgrade_config(config, alt_config_path)
        verify_config_colors(config)

    else:
        logging.debug("Configuration file not found, installing alog...")
        config = install()
