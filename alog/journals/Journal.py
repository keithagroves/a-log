...
from alog.config import validate_journal_name
...
class Journal:
    ...
    def _update_semantic_index(self) -> None:
        ...
        try:
            from alog.search import update_index
    ...
    def validate_parsing(self) -> bool:
        """Confirms that the alog is still parsed correctly after conversion to text."""
...
class LegacyJournal(Journal):
    """Legacy class to support opening journals formatted with the alog 1.x
...
def open_journal(journal_name: str, config: dict, legacy: bool = False) -> Journal:
    ...
    backwards compatibility with alog 1.x
    """
    ...
    if os.path.isdir(config["journal"]):
        ...
        if config["journal"].strip("/").endswith(".dayone") or "entries" in os.listdir(
            ...
            from alog.journals import DayOne

            return DayOne(**config).open()
        else:
            from alog.journals import Folder
