...
from alog.config import get_journal_name
from alog.config import scope_config
from alog.editor import get_text_from_editor
...
def run(args: "Namespace"):
    ...
    config = install.load_or_install_alog(args.config_file_path)
...
def append_mode(args: "Namespace", config: dict, journal: "Journal", **kwargs) -> None:
    ...
    if template_text is not None and raw == template_text:
        ...
        raise AlogException(Message(MsgText.NoChangesToTemplate, MsgStyle.NORMAL))
...
def _semantic_search(args: "Namespace", journal: "Journal", config: dict) -> None:
    from alog.search import semantic_search
