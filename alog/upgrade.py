...
from alog.config import is_config_json
...
from alog.output import print_msgs
...
def backup(filename: str, binary: bool = False):
    ...
    try:
        ...
        if not cont:
            raise AlogException(Message(MsgText.UpgradeAborted, MsgStyle.WARNING))
...
def upgrade_alog(config_path: str) -> None:
    config = load_config(config_path)

    print_msg(Message(MsgText.WelcomeToAlog, MsgStyle.NORMAL, {"version": __version__}))
    ...
    for journal_name, path in encrypted_journals.items():
        ...
        new_journal.config["encrypt"] = "alogv2"
        new_journal._get_encryption_method()
        # Copy over password (alogv1 only supported password-based encryption)
